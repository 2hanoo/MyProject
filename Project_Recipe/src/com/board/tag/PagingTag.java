...
public class PagingTag extends TagSupport {
	...
	public String getPaging(){
		...
		/*
		lastPage(총페이지 수)는 total이 11이고 
		list_size  가 10이면 1을 가진다.
		그래서 총 페이지 수가 나누어 떨어지지않으면 나머지 레코드를 출력할 페이지가 필요하다.
		*/
		if(total%list_size !=0 ) lastPage = lastPage +1;
//		currentlast 가 lastPage(총 페이지 수 )보다 크면 마지막 페이지로 수정
		...
		if (page>1) {
			...
			ret += " <a href = \"javascript:goPage('"+prevLast+"')\"><span><img src='../image/common/btn_paginate_prev.gif' alt = '이전' /></span></a>";
		}else{
